...
void motion(int x, int y){
    ...
    pProjX = (bRG - bBClick) / (mBClick - mRG);
    pProjY = mBClick * pProjX + bBClick;

    float proportionB = (dist(fX, fY, pBx, pBy) / dist(pProjX, pProjY, pBx, pBy));
    float distanceRG = dist(pGx, pGy, pRx, pRy);
    gB = 1.0 - proportionB;
    gR = (1.0 - ( dist(pProjX, pProjY, pRx, pRy) / distanceRG )) * proportionB;
    gG = (1.0 - ( dist(pProjX, pProjY, pGx, pGy) / distanceRG )) * proportionB;
